...
!   8/18 if test changed to handle case where nphonatonnode is zero
!
!  Conditions of use:
!
...
!
  ...
  if (status/=0) call outofmemory('phonond','mcvnptr')
...
!
    if (numat.ne.nphonat.and.nphonatonnode.gt.0) then
      if (atom2local(nphonatonnodeptr(nphonatonnode)).ne.nphonatonnode) then
!*********************************************************************
...
!
            if (iloc.gt.0) then
              ...
              iy = ix + 1
              iz = ix + 2
              dtmp(1:mcv+msv,1) = dervi(1:mcv+msv,ix)
              dtmp(1:mcv+msv,2) = dervi(1:mcv+msv,iy)
              dtmp(1:mcv+msv,3) = dervi(1:mcv+msv,iz)
!
!  Post send
!
              call MPI_ISend(dtmp,ntmp,MPI_double_precision,0, &
...
!
        if (lgrueneisen) then
          call gruneisengd(k-nlkpt+1_i4,mcv,mcvloc,mcvptr,msv,nphonatc,eigr,maxeigr, &
                           derv2,maxd2,rmass,fscale,lprint)
        endif
!
...
!
        call peigengd(mcv,mcvloc,mcvptr,mcvnptr,mcvrptr,freq(1,k-nlkpt+1),ncore,nphonatc,nphonatc,nphonats, &
...
!  Output frequencies / DOS / intensities
!
          call peigend(mcv,mcvloc,mcvptr,mcvnptr,mcvrptr,freq(1,k-nlkpt+1),ncore,nphonatc,nphonatc,nphonats, &
...
!
          if (lgrueneisen) then
            call gruneisend(k-nlkpt+1_i4,mcv,mcvloc,mcvptr,msv,nphonatc,eigc,maxeigc,derv2,dervi,maxd2, &
                            rmass,fscale,lprint)
          endif
!
!  Lower symmetry to remove imaginary modes if selected
...
!
  ...
  if (status/=0) call deallocate_error('phonond','mcvnptr')
