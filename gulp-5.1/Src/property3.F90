...
!
    eltmp(1:6,1:6) = elcon(1:6,1:6)
    call dsyev('N','U',6_i4,eltmp,6_i4,elconeig,eltmp2,36_i4,ifail)
...
!**********************
  if (lprint.and.ioproc) then
    if (lelastic.and.abs(elcon(1,1)).gt.zero) then
      ...
      if (lelconeigok) then
        if (index(keyword,'oldu').ne.0) then
          write(ioout,'(/,''  Elastic Constant Tensor Eigenvalues: (Units=10**11 Dyne/cm**2= 10 GPa)'',/)')
        else
          write(ioout,'(/,''  Elastic Constant Tensor Eigenvalues: (Units=GPa)'',/)')
        endif
        write(ioout,'(''-------------------------------------------------------------------------------'')')
        if (index(keyword,'oldu').ne.0) then
          write(ioout,'(4x,'' 1 = '',f10.5,4x,'' 2 = '',f10.5,4x,'' 3 = '',f10.5)') &
                (0.1_dp*elconeig(i),i=1,3)
          write(ioout,'(4x,'' 4 = '',f10.5,4x,'' 5 = '',f10.5,4x,'' 6 = '',f10.5)') &
                (0.1_dp*elconeig(3+i),i=1,3)
        else
          write(ioout,'(4x,'' 1 = '',f10.4,4x,'' 2 = '',f10.4,4x,'' 3 = '',f10.4)') &
                (elconeig(i),i=1,3)
          write(ioout,'(4x,'' 4 = '',f10.4,4x,'' 5 = '',f10.4,4x,'' 6 = '',f10.4)') &
                (elconeig(3+i),i=1,3)
        endif
        write(ioout,'(''-------------------------------------------------------------------------------'')')
