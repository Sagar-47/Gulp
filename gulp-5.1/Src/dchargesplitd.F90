            ...
            do nbi = 1,nbonds(i)
              ...
              if (nbqo.gt.0) then
                ...
                if (lstrain) then
                  ...
                  dqs(nbq,1) = dqs(nbq,1) - strm1*xrk(iv)*xrk(iv) - strm2
                  dqs(nbq,2) = dqs(nbq,2) - strm1*yrk(iv)*yrk(iv) - strm2
                  dqs(nbq,3) = dqs(nbq,3) - strm1*zrk(iv)*zrk(iv) - strm2
                  dqs(nbq,4) = dqs(nbq,4) - strm1*yrk(iv)*zrk(iv)
                  dqs(nbq,5) = dqs(nbq,5) - strm1*xrk(iv)*zrk(iv)
                  dqs(nbq,6) = dqs(nbq,6) - strm1*xrk(iv)*yrk(iv)
                endif
                ...
                do nbi = 1,nbonds(i)
                  ...
                  if (nbqo.gt.0) then
                    ...
                    if (lstrain) then
                      ...
                      dqs(nbq,1) = dqs(nbq,1) - strm1*xrk(iv)*xrk(iv) - strm2
                      dqs(nbq,2) = dqs(nbq,2) - strm1*yrk(iv)*yrk(iv) - strm2
                      dqs(nbq,3) = dqs(nbq,3) - strm1*xrk(iv)*yrk(iv)
                    endif
                  elseif (nbqo.lt.0) then
                    dqx(nbq) = dqx(nbq) - sineq*xrk(iv)*qlj
                    dqy(nbq) = dqy(nbq) - sineq*yrk(iv)*qlj
...
!
              do nbi = 1,nbonds(i)
                ...
                if (nbqo.gt.0) then
                  ...
                  if (lstrain) then
                    if (ndim.eq.3) then
                      dqs(nbq,1) = dqs(nbq,1) + qljj*dtrm1i*rxk*rxk
                      dqs(nbq,2) = dqs(nbq,2) + qljj*dtrm1i*ryk*ryk
                      dqs(nbq,3) = dqs(nbq,3) + qljj*dtrm1i*rzk*rzk
                      dqs(nbq,4) = dqs(nbq,4) + qljj*dtrm1i*ryk*rzk
                      dqs(nbq,5) = dqs(nbq,5) + qljj*dtrm1i*rxk*rzk
                      dqs(nbq,6) = dqs(nbq,6) + qljj*dtrm1i*rxk*ryk
                    elseif (ndim.eq.2) then
                      dqs(nbq,1) = dqs(nbq,1) + qljj*dtrm1i*rxk*rxk
                      dqs(nbq,2) = dqs(nbq,2) + qljj*dtrm1i*ryk*ryk
                      dqs(nbq,3) = dqs(nbq,3) + qljj*dtrm1i*rxk*ryk
                    endif
                  endif
                elseif (nbqo.lt.0) then
                  dqx(nbq) = dqx(nbq) + qljj*dtrm1i*rxk
...
!
              ...
              do nbi = 1,nbonds(i)
                ...
                if (nbqo.gt.0) then
                  ...
                  if (lstrain) then
                    dqs(nbq,1) = dqs(nbq,1) + qljj*dtrm1i*rx*rx
                  endif
                elseif (nbqo.lt.0) then
                  dqx(nbq) = dqx(nbq) + qljj*dtrm1i*rx
                  dqy(nbq) = dqy(nbq) + qljj*dtrm1i*ry
