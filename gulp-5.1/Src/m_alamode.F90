...
!
    character(len=132),                         save :: alamodedir
    character(len=132),                         save :: almexe
    character(len=132),                         save :: anphonexe
...
#ifdef MPI
  use parallel,       only : nprocs, MPI_comm_GULP
#endif
...
!  Local variables
!
  character(len=80)                                :: alminput
  character(len=80)                                :: almoutput
  character(len=80)                                :: dispfile_c
  character(len=80)                                :: dispfile_h
  character(len=80)                                :: forcefile_c
  character(len=80)                                :: forcefile_h
  character(len=132)                               :: line
  character(len=80)                                :: prefix
  character(len=132)                               :: runalm
  integer(i4)                                      :: i
#ifdef MPI
  integer(i4)                                      :: ier
#endif
...
!
    runalm = trim(almexe)//' '//trim(alminput)//' > alm.log'
...
!
      call MPI_bcast(ndisp2,1,MPI_integer,0,MPI_comm_GULP,ier)
    endif
#endif
!***********************************************
...
!  Broadcast displacement patterns
!
        call MPI_bcast(ni,1,MPI_integer,0,MPI_comm_GULP,ier)
...
!  Broadcast displacement patterns
!
      call MPI_bcast(ndisp3,1,MPI_integer,0,MPI_comm_GULP,ier)
    endif
#endif
!***********************************************
!  Loop over displacements and compute forces  *
...
!
  ...
  use configurations, only : ntempstp, tempstp, tempcfg
  use control
  use current
...
!  Local variables
!
  character(len=80)                                :: anphoninput
  character(len=80)                                :: anphonoutput
  character(len=80)                                :: fcsfile
  character(len=80)                                :: line
  character(len=80)                                :: prefix
  character(len=5)                                 :: procs
  character(len=132)                               :: runanphon
  integer(i4)                                      :: i
  integer(i4)                                      :: ind1
  integer(i4)                                      :: ind2
  integer(i4)                                      :: j
...
!
    runanphon = 'grep Primitive alm.log | tail -n 1 > almgrep.tmp'
!
...
!
    ...
    if (abs(ntempstp(ncf)).gt.0) then
      tmin = tempcfg(ncf)
      tmax = tempcfg(ncf) + dble(ntempstp(ncf))*tempstp(ncf)
      if (tmax.gt.tmin) then
        write(ioalamode,'(''  TMIN = '',f12.4)') tmin
        write(ioalamode,'(''  TMAX = '',f12.4)') tmax
        write(ioalamode,'(''  DT =   '',f12.4)') tempstp(ncf)
      else
