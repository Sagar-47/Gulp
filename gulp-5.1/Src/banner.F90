...
!
!  Julian Gale, Curtin University, April 2019
!
  use iochannels
  use parallel
#ifdef TRACE
...
!******************
  if (ioproc) then
#ifdef ACCELRYS
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                       GENERAL UTILITY LATTICE PROGRAM  v5.1                  *'')')
    ...
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                     BIOVIA Materials Studio 2019 Release                     *'')')
    write(ioout,'(''********************************************************************************'')')
#else
    write(ioout,'(''********************************************************************************'')')
    ...
    write(ioout,'(''*                    Curtin University, Western Australia                      *'')')
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''* Version = 5.1.1 * Last modified =  11th April 2019                           *'')')
    write(ioout,'(''********************************************************************************'')')
#endif
  endif
#ifdef TRACE
