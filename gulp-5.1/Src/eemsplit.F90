...
!
  do i = 1,numat
    ni = nat(i)
    qi = qf(i)
    if (lelementOK(ni).and.nregionno(nsft+nrelat(i)).eq.1) then
      neem = neem + 1
      neemptr(neem) = i
      neemrptr(i) = neem
...
!
    do i = 1,neembond
      ib1 = neembonded(1,i)
      ib2 = neembonded(2,i)
      z(i) = z(i) - extpotcfg(nsft+nrelat(ib1))
      z(i) = z(i) + extpotcfg(nsft+nrelat(ib2))
    enddo
    if (lmultiqrange) then
      do i = 1,neembond
        ib1 = neembonded(1,i)
...
!
    eself = eself + qi*extpotcfg(nsft+nrelat(ib1))
    eself = eself - qi*extpotcfg(nsft+nrelat(ib2))
!
    nregioni = nregionno(nsft+nrelat(ib1))
    eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eself - eself_before
    nregioni = nregionno(nsft+nrelat(ib2))
    eregion2region(nregioni,nregioni) = eregion2region(nregioni,nregioni) + eself - eself_before
!
