...
!
  use configurations
  use control,        only : lmodco, lstraincell
  use current
...
!
  if (ndim.gt.0) then
    ...
    if (.not.loptcellpar) then
      if (lstraincell) then
!
!  Apply existing strain
!
        if (ndim.eq.3) then
          call strain3D(straincfg(1,ncf),rv)
        elseif (ndim.eq.2) then
          call strain2D(straincfg(1,ncf),rv)
        elseif (ndim.eq.1) then
          call strain1D(straincfg(1,ncf),rv)
        endif
      endif
      do i = 1,nstrains
        x0(i) = 1.0_dp
