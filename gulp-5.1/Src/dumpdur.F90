...
!
        ...
        if (lstraincell) then
!
!  For cell strain output vectors to preserve Cartesian orientation
!
          if (ndimen(nc).eq.3) then
            write(iout,'(''vectors '')')
            write(iout,'(3f12.6)') (rvcfg(j,1,nc),j=1,3)
            write(iout,'(3f12.6)') (rvcfg(j,2,nc),j=1,3)
            write(iout,'(3f12.6)') (rvcfg(j,3,nc),j=1,3)
            write(iout,'(6i2)') (itmp(j),j=1,6)
            write(iout,'(''cellstrain'',6(1x,f10.6))') (straincfg(j,nc),j=1,6)
          elseif (ndimen(nc).eq.2) then
            write(iout,'(''svectors '')')
            write(iout,'(2f12.6)') (rvcfg(j,1,nc),j=1,2)
            write(iout,'(2f12.6)') (rvcfg(j,2,nc),j=1,2)
            write(iout,'(3i2)') (itmp(j),j=1,3)
            write(iout,'(''cellstrain'',3(1x,f10.6))') (straincfg(j,nc),j=1,3)
          elseif (ndimen(nc).eq.1) then
            write(iout,'(''pvectors '')')
            write(iout,'(f12.6)') rvcfg(1,1,nc)
            write(iout,'(1i2)') itmp(1)
            write(iout,'(''cellstrain'',1x,f10.6)') straincfg(1,nc)
          endif
        else
!
!  Normal cell output
...
!
    if (linitcfg(nc)) then
      ...
      do i = 1,numat
        write(iout,'(3(f15.9,1x))') xinitcfg(nlsft+i),yinitcfg(nlsft+i),zinitcfg(nlsft+i)
      enddo
    endif
!
!  QM/MM mode
